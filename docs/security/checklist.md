...
- [ ] No `os.getenv` calls outside `padhanam/config/` (verified by AST enforcement test).
- [ ] No new `.env` reads outside `padhanam/config/`.
...
- [ ] All TLS-bearing services read TLS config through `padhanam/config/` (when applicable in this session).
