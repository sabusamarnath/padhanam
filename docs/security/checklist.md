...
- [ ] No `os.getenv` calls outside `zephyr/config/` (verified by AST enforcement test).
- [ ] No new `.env` reads outside `zephyr/config/`.
...
- [ ] All TLS-bearing services read TLS config through `zephyr/config/` (when applicable in this session).
