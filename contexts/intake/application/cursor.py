"""Cursor codec for the intake list surface (D127).

Base64-encoded JSON, opaque to consumers, mirroring the portfolio
cursor pattern. ``list_intakes`` is the one paginated intake
surface; the codec round-trips ``IntakeListCursor`` across the HTTP
and CLI boundaries verbatim.
...
def encode_intake_cursor(cursor: IntakeListCursor) -> str:
    return _encode(
        {
...
def decode_intake_cursor(encoded: str) -> IntakeListCursor:
...
__all__ = ["decode_intake_cursor", "encode_intake_cursor"]
