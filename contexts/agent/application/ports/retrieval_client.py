"""AgentRetrievalClient Protocol port + RetrievedChunk DTO (D88).
...
and returning chunks), and the wiring adapter at
``apps/cli/_cross_context.py`` translates against the ingestion
context's split ``search_vector`` / ``traverse_graph`` methods per
D5 / D65's explicit decision that hybrid composition is an agent-
layer concern.
...
class AgentRetrievalClient(Protocol):
    """Callable port for the agent runtime's retrieval surface (D88).

    The Protocol is structurally typed: any callable accepting the
    named keyword arguments and returning a ``tuple[RetrievedChunk, ...]``
