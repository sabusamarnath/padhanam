"""Cursor codec for the portfolio read surface (D124).

Base64-encoded JSON, opaque to consumers, mirroring the run-history
and optimization cursor patterns. ``list_cases`` is the one
paginated surface; the codec round-trips ``CaseListCursor`` across
the HTTP and CLI boundaries verbatim.
...
def encode_case_cursor(cursor: CaseListCursor) -> str:
    return _encode(
        {
...
def decode_case_cursor(encoded: str) -> CaseListCursor:
...
__all__ = ["decode_case_cursor", "encode_case_cursor"]
