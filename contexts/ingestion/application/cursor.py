"""Cursor codec for the source-list read surface (D104, S38).

Encode and decode ``SourceListCursor`` instances as opaque
strings that survive the HTTP boundary at S38. Mirror of the
audit and run-history precedents — base64-of-JSON per D104's
choice text:

    {"created_at": "2026-05-13T10:30:00.000000+00:00",
...
at S38 catches the error and returns 400.
"""
...
def encode(cursor: SourceListCursor) -> str:
    """Encode a ``SourceListCursor`` as a base64-of-JSON string."""
    payload = {
...
def decode(encoded: str) -> SourceListCursor:
    """Reconstruct a ``SourceListCursor`` from an opaque string.
