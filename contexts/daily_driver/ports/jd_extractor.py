...
and parses the response into an ``ExtractedQualification``.
"""
...
class JdExtractorPort(Protocol):
    """Extract three qualification-field drafts from a job description (S103ad, D236)."""

    async def extract(self, *, jd_text: str) -> ExtractedQualification | None:
        """Return the drafted fields for the job description, or ``None`` when the
