...
- S11: Per-tenant connection resolution and migration runner. Routing layer in `zephyr/config/` reads registry, decrypts credentials (calling the operator-context `reveal_connection_config` use case as the system actor framing landed in D34), returns per-tenant AsyncSession factory. Per-tenant Alembic track wired as a second `[tenant]` named section under the existing `alembic.ini`. `make migrate` runs both phases in order (control-plane first, per-tenant second). Per-tenant initial schema applied. D-entry for migration runner shape.
