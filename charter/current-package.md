...
## P8 active — S28b closed; S29b next
...
**S29b next.** SSE streaming and trace capture for agent runtime. FastAPI SSE endpoint for agent invocation. OTel span attributes extending D49 and D57. Streaming-aware trace capture. Per-invocation cost roll-up. S28b's substrate (the tool registry generalisation, the two thin ports as a Phase 1 norm, the classification-to-invariant mapping at the invocation boundary) holds Padhanam-as-intelligence-layer in code at Phase 1; S29b extends the observability surface; S30b lands the operator-facing `padhanam agent run` CLI and the LVT PM agent end-to-end demonstration with operator-uploaded sources including a drafted-artifact-as-deliverable shape exercising the intelligence-layer commitment in product form.
