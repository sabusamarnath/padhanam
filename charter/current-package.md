...
## P9 active — S35 in flight (P9 close)
...
**S35 in flight** framing P9 close via end-to-end demonstration. Five-commit shape per `briefs/p9/s35.md`: charter updates ahead of code (this commit) carrying two new deferred-decisions entries (integration-test slowness from real-LLM inference; HTTP API for ingestion management absorbed from P6 and not landed at P9) plus the investigation note `docs/notes/integration-test-slowness-investigation.md`; carryover fix for the `test_agent_sse_endpoint.py` `_runtime_with_script` helper missing the `run_history_writer` field at the `AgentRuntimeComposition` constructor; end-to-end demonstration trigger against tenant_a exercising agent invocation → SSE event stream → run write through the S31 substrate → citation linkage through S32 → audit-chain hash verification → HTTP read surface through S34, recorded at `docs/smoke/p9_s35_e2e_demo.md`; P9 retrospective addendum to `charter/packages/p9-epic.md` and P9 entry at `log/packages.md` per the P8 precedent, plus archive of the epic to `docs/archive/packages/p9.md`; session-log entry and current-package transition to "P9 closed; P10 framing next". Pre-write reconciliation outcomes at session open: tenant registry empty per the recurring wipe class (S30b/S31 carryover) — demo step 1 is `make seed-tenants` recovery; tenant_a's two existing agents (Flowstate ProblemFramer v1/v2) ship with empty `tool_allowlist` per the P8→P9 carryover, so a new agent revision with `tool_allowlist=["retrieval"]` is seeded ahead of the demo (option (a) from the brief's surface 2); Langfuse-web returns 200 OK so the optional trace-correlation step in the smoke is exercised. No new D-entries expected at S35; all P9 substrate decisions landed at D94 through D98.
