...
**S32 in flight.** D96 lands at session-open with four shapes the build session lands: citation candidate value objects at `contexts/agent/domain/citation_candidates.py` (`ChunkCitationCandidate` carrying chunk identity plus content and source snapshot; `EntityCitationCandidate` carrying the Neo4j composite plus `source_chunk_ids` provenance); `ToolCallCompleted` extension carrying `citation_candidates: tuple[CitationCandidate, ...] = ()` so the citation surface rides the existing event vocabulary without a new event type; schema revisions at `run_chunk_citations` (drop `source_citation text`, add `source_snapshot jsonb`) and `run_entity_citations` (drop `entity_display_label text`, add `source_chunk_ids text[]`) keeping render shape a Phase 2 read-time concern over structured snapshots; `PostgresRunHistoryAdapter` grows its `async with session.begin()` block to write runs + chunk citations + entity citations within a single transaction with partial-failure rollback semantics. The `AgentRetrievalClient` port grows a `RetrievalResult` envelope so the same adapter produces both LLM-facing `RetrievedChunk` projections and citation candidates from the same ingestion-side `ChunkResult`/`EntityResult` single-pass. `invoke_agent` accumulates citation candidates from `ToolCallCompleted` events across the run with within-run deduplication first-seen-wins per `(chunk_id, run_id)` and `(entity_tenant_id, entity_name, entity_type, run_id)`. Tenant-isolation contract harness extends for citation rows. Live-stack smoke at session close verifies one runs row plus at least one citation row on `tenant_a` with `source_snapshot` populated.
