...
## S35a in flight (P9 carryover hygiene)

P9 closed at S35 on 2026-05-14 with end-to-end demonstration against tenant_a, but four hygiene workstreams identified at S35 close did not land within P9's natural boundary. S35a absorbs them: (1) live-LLM tier separation per the slowness investigation note's recommendation (D99 lands the marker-based convention deferred at S35); (2) CLI composition-root settings threading to unblock the 11 methodology/tool CLI test failures and close the apps/api-vs-apps/cli architectural asymmetry (D100); (3) tenant registry actor provenance via a new `created_by_user_id` column so the S30b wipe-guard pattern works symmetrically with the existing five protected control-plane tables (D101); (4) trace_id propagation from the active OTel span to the runs row, closing the operational gap S35's end-to-end demo surfaced (D27's join-key claim's first end-to-end exercise). S35a does not re-open P9; it closes hygiene that ran past P9's natural boundary. P9 retrospective and archive stand from S35. P10 framing remains the next strategic-mode conversation after S35a close.
