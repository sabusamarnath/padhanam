...
**Pre-P12 hygiene session in flight.** Seven items consolidating P11 carryover: README expansion per four operator-confirmed dispositions (methodology framing as living hypothesis, schema.md and deferred-decisions.md in charter reading order, Apache 2.0 strategic commitment prominence, codebase visibility and operational orientation sections); methodology-candidates consolidation into new `charter/p12-audit-inputs.md`; `make build-api` Makefile target hardening (already fixed at S42 commit ac5a6bf; verification step rather than new fix); graph-extract pipeline reliability investigation with bounded-fix-or-document disposition; CLI/API wiring-integration-test parity verification with backfill if absent; S37 audit reader validator loosening for pre-existing empty correlation_id rows; stray `apps/api/{routers}/` directory cleanup. Session shape is hygiene rather than substrate or bridge; commit prefixes mix `docs(p11/hygiene)`, `chore(p11/hygiene)`, `fix(p11/hygiene)`, and `docs(charter)` per work nature.
