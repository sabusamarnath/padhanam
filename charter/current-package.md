...
S33 in flight: lands the UX-shaped query port and the Postgres adapter at the run-history context. Port commits to two methods (`get_run` returning `RunRecord | None` as the aggregate per D97's RunRecord-as-aggregate shape; `list_runs_with_filters` returning a cursor-paginated `RunListPage`), a four-filter list vocabulary (agent template id and version, started_at range, termination reason), cursor pagination on `(started_at, id)` for stable scroll-back, and the storage-versus-render discipline at the read surface (domain types pass straight through; the consumer projects render shape). Port location follows the S22 `RetrievalClient` producer-side-with-consumer-shape precedent rather than S31's consumer-defined `RunHistoryWriter` precedent because the consumer (HTTP API at S34/S35) is a composition surface rather than a bounded context. Read-DTO symmetry with the write side eliminates the brief's drafted `RunWithCitations` wrapper: `RunRecord` already aggregates citations from S32's D96 extension, so the read returns the same record type the writer persists.
