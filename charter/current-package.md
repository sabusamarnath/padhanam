...
## S35b in flight (P9 carryover hygiene continued)

**S35b opens** at 2026-05-14 as a second P9-carryover hygiene session, framed around three workstreams against the six pre-existing failures named at S35a close: (1) TRUNCATE+CASCADE fix on the chunks-to-`run_chunk_citations` FK boundary introduced at S32 (two failing fixtures); (2) SQLAlchemy-based truncation in the tenant-isolation ingestion contract test, migrating from the `docker compose exec psql` shell-out that the padhanam-api image cannot satisfy, mirroring the methodology-fixture pattern at `test_methodology_isolation.py` while populating non-empty test data so the harness is pass-because-isolated rather than pass-because-empty (two failing tests); (3) `test_agent_streaming_isolation` diagnostic absorbed at session-open reconciliation surface 3 — the failure is `AgentRuntimeComposition` missing `run_history_writer`, identical to the S35 carryover fix on `test_agent_sse_endpoint.py::_runtime_with_script`, pure test-scaffolding staleness not an isolation gap (two failing tests; operator confirmed fold-in). No new D-entries expected; D24 (tenant-isolation contract harness discipline) already commits the principle the fixes restore.
