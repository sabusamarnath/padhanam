...
**S40 in flight.** Retrieval-evaluation runner work per D108's second commitment and D110's seven commitments: runner co-located with gold-set substrate at `contexts/retrieval_evaluation/`, `EvaluationRun` aggregate root with status lifecycle, per-query plus per-strategy-aggregated result records, agent-level `AgentRetrievalClientAdapter` invocation (single dispatch site for strategy translation per S40 pre-write reconciliation Finding 3), audit-context-event-level tamper-evidence absorption establishing the platform-computed-records regime per D110 commitment 7's three-regime distinction. Runner ships against the two D66 strategies with executing branches at the adapter (`vector_only`, `graph_only`); `parallel_rrf` deferred to a Phase 2 fusion-implementation session per the new `charter/deferred-decisions.md` entry (S40 pre-write reconciliation Finding 2). Gold-set aggregate-level audit emission gap surfaced at S40 pre-write reconciliation Finding 6 and lands as a separate `charter/deferred-decisions.md` carryover activating no later than Phase 1 close. Smoke exercises the S39b real-corpus gold-set against tenant_a; the LVT-feedback-loop caveat and CC-autonomously-authoring provenance caveat from S39b carry forward into the smoke document explicitly. Carryover decision deferred to S40 close: whether the S39b gold-set's metrics are usable as S41 evidence, or whether an S40b clean-gold-set bridge session is required before S41 ships.
