...
**S41 in flight.** Optimization layer substrate per D108's four-category commitment and D111's eight commitments: `contexts/optimization/` as new bounded context completing the four-context P11 substrate scaffold; `OptimizationRun` aggregate root mirroring the EvaluationRun shape from D110 commitment 2 (engine-invocation lifecycle plus `skipped_categories` JSONB carrying Phase 1 substrate-gap transparency for `model_choice` and `prompt_revision`); `Recommendation` aggregate with discriminated evidence citations (structured caveat-annotation field per the S40b graph_only-empty pattern), NOT NULL FK to optimization_runs, append-only text-and-citation content; `RecommendationRule` and `MetricCalculator` pluggable domain abstractions operationalising the new vendor-flexibility principle at `charter/principles.md`; four default rule implementations with retrieval_strategy and cost_optimization producing substantive recommendations and model_choice/prompt_revision producing structured zero-recommendation skip-reasons captured on the OptimizationRun aggregate; audit-chain-absorbed lifecycle across both aggregates per D111 commitment 8 extending D110 commitment 7's regime. Smoke exercises the engine against tenant_a using S40b's run `c168c2ba` as evaluation evidence and tenant_a's run-history aggregates as cost evidence; retrieval_strategy and cost_optimization recommendations cite producer-context records end-to-end.
