...
**S29b closed on 2026-05-12.** Ten commits landing the streaming runtime substrate. D90 absorbs four sub-choices (event vocabulary at domain layer, nested-trace-simple-audit asymmetry per Option C, streaming-only executor per Option A, transport-neutral domain placement per Option A) plus three S29b pre-write reconciliation outcomes (URL stays principal-derived per existing apps/api convention; method names preserved as `AgentExecutor.execute` and `InferencePort.stream_complete` for symmetry). Eleven domain-layer event types at `contexts/agent/domain/events.py` (InvocationStarted, IterationStarted, LLMCallStarted, ContentDelta, ToolCallProposed, ToolCallExecuting, ToolCallCompleted, IterationCompleted, InvocationCompleted, InvocationFailed, InvariantBlocked) expose the runtime's state machine to consumers as the transport-neutral substrate the gallery pre-population at Phase 2 demos against; `AgentEvent` discriminated union and `TERMINAL_EVENT_TYPES` helper. `AgentExecutor.execute(context)` returns `AsyncIterator[AgentEvent]`; `AgentResult` derived via `collect_to_result` helper at `contexts/agent/application/collect.py` for non-streaming callers; TerminationReason and AgentResult relocated to `contexts/agent/domain/` for hexagonal cleanness with back-compat re-exports on the ports layer. `InferencePort.stream_complete` returns `AsyncIterator[CompletionChunk]`; LiteLLM adapter implements via `litellm.acompletion(stream=True)` with piecewise tool-call accumulation and `stream_chunk_builder` reassembly for usage extraction. AgentLoopExecutor as async generator yielding events at six per-loop granularity points; three terminal-event types map to TerminationReason (CONTENT, MAX_ITERATIONS, TOOL_NOT_REGISTERED, ERROR) plus the dedicated INVARIANT_BLOCKED terminal event. Nested OTel span hierarchy (agent.invocation → agent.iteration → gen_ai.llm_call + agent.tool_call) with cost roll-up across levels per D49 extended; D57's cost-query two-layer abstraction continues to work. SSE endpoint at `apps/api/routers/agent.py` (POST /agents/{agent_id}/invoke; principal-derived tenant) translates events to W3C EventSource wire format via `apps/api/adapters/sse_event_translator.py`; `AgentRuntimeComposition` dataclass on app.state.agent_runtime. The transport-neutral domain placement is the fourth reinforcement of the consumer-port-plus-wiring-adapter pattern (after S26a-1, S26a-2, S27b, S28b) — Phase 1 norm. Pre-write reconciliation fires its sixth instance with three brief assumptions resolved at one user-question moment (apps/api existence, URL shape, method names); the captures entry at `log/captures.md` 2026-05-12 names this pattern as a Phase 1 close audit methodology-promotion candidate. 732 tests pass at session close (up from 709 at S28b close); 24 import-linter contracts kept; no schema changes (behavioural only); the live-stack end-to-end test is structurally correct against the new shapes but requires a padhanam-api image rebuild (the running container at session-end holds a pre-S28b image) — rebuild queued as S30b setup. Brief preserved at `briefs/p8/s29b.md`.

**S30b in flight.** S30b is the activating session that proves the agent runtime substrate produces drafted artifacts a human acts on, per the S28b intelligence-layer commitment. The `padhanam agent run` CLI consumes the SSE endpoint and renders the streaming event vocabulary to terminal; two end-to-end demonstrations exercise the substrate against real operator-uploaded source material at narrow and broad artifact scales through the same runtime — a Flowstate-McKinsey ProblemFramer agent on tenant alpha producing a SMART problem statement (narrow artifact), a Forgepath-LVT LVTGuide agent on tenant beta producing a full Lean Value Tree (broad artifact). Different tenants exercise the principal-derived tenant resolution path for both invocations. The two-demo comparison is the proof point for the bet's product-shape commitment: the same substrate that streams a one-paragraph problem framing also streams a multi-level value-tree decomposition, with retrieval, audit, and per-invocation cost roll-up unchanged across the two artifact scales. P8 close at S30b lands the close archive at `docs/archive/packages/p8.md` and the measured-outcomes paragraph at `log/packages.md`.
