...
## P8 active — S26b closed; S27b next
...
**S27b next.** AgentExecutor port definition at `contexts/agent/ports/agent_executor.py`; AgentLoopExecutor adapter at `contexts/agent/adapters/outbound/agent_loop_executor.py` (hand-rolled per D84); `invoke_agent` use case; LLM-with-tool-loop control flow against LiteLLM per D4 and retrieval per D60/D66; cost capture per D49 and D57 extending to per-agent-invocation cost; audit events per D26; tenant-isolation contract test extension. S27b also commits the override-mode resolver semantics deferred at D87 (how augment concatenates, how tighten intersects, how replace substitutes at agent invocation time).
