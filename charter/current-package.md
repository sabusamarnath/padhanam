...
**S40b in flight.** Verification-and-hygiene bridge between S40 runner close and S41 optimization framing. Corpus refresh on tenant_a to remove the LVTGuide-system-prompt overlap S40 close surfaced (the contaminated `lvt_methodology_overview.md` source replaced with a Pacelane recovery-first wearable case study whose vocabulary diverges from the system prompt's strategic-hierarchy framing), clean gold-set authoring via content-fit selection (CC reads each retrieved chunk and judges answer-fit rather than selecting rank-1), S39b rank-selected gold-set rename for disambiguation, verification re-run of the S40 runner against the new gold-set to confirm contamination broke. Reframed Query 2 (`"how did Pacelane apply LVT to its recovery-first wearable launch"`) replaces the contaminated S39b Query 2; Queries 1 and 3 stay unchanged. Corpus fixtures lifted into version control at `tests/fixtures/corpus/p11_s40b/` so chunk references are reproducible across DB rebuilds (closing the S39b corpus-not-in-repo gap prospectively for S40b; S39b's corpus stays uncommitted as a separate P12 audit input). No new D-entries; the methodology principle the session demonstrates — selection function at authoring must differ from the function being evaluated — is forwarded to P12 audit as a candidate, not committed as a D-entry inside S40b.
