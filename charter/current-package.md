...
**S42 in flight.** HTTP transport surface for the retrieval_evaluation and optimization producer-context substrates per D108 and D112. Routes ship for retrieval_evaluation (full including two-step gold-set discovery, evaluation run kickoff and inspection) and optimization (full including lifecycle write routes and optimization run kickoff); run_history HTTP surface shipped at S34 per D98 and audit HTTP surface shipped at S37 per D103, so S42 adds no work there (Finding 1 investigation outcome). Pydantic v2 DTOs with discriminated-union evidence_citations under the existing flat-module router convention (`apps/api/routers/<context>.py` plus `_<context>_dto.py` plus `_<context>_query.py`; the Finding 5 deviation from the brief's per-context-subdirectory framing was caught at session open). JWT-resolved tenancy per S15/S37 hardening; cursor pagination matching the reader-layer shape; OpenAPI specification as procurement-grade Phase 2 UX-consumer documentation. Smoke verifies the full surface end-to-end against the production-shaped API container using `make build-api` per the post-S41 tooling.
