...
- S4: Langfuse 3 in Compose behind Caddy (six-service deployment: langfuse-web, langfuse-worker, langfuse-db, clickhouse, redis [shared with existing Quorum Redis via DB number per D18], minio). Headless project and API key bootstrap via LANGFUSE_INIT_*. Charter updates: D15, D16, D17, D18, principles bounded-contexts addition, this current-package block.
