...
## P8 active — S26a-2 in flight; S26b next
...
**S26a-2 in flight (opened 2026-05-12).** Per-tenant work closing S26a: per-tenant Alembic `0009_agent_role_lineage` adding `source_role_id` + `source_role_version` to `agent_templates` with paired-null CHECK and PM-agent backfill; `AgentTemplate` domain gains the role lineage pair (paired-null invariant per D86 alongside D75's methodology pair); `MethodologyView` consumer-side DTO extended to carry resolved `role_id` + `role_version` so `create_agent_from_methodology` populates both lineage pairs from a single cross-context hop (pre-write reconciliation: brief assumption that the view already carried these proved wrong; extension is the cleanest path); new `RoleLookup` port + `RoleView` DTO at `contexts/agent/application/ports/`; new `create_agent_from_role` use case; `RoleLookupAdapter` at `apps/cli/_cross_context.py`; `padhanam role` CLI namespace (create, get, list, update, archive) plus `padhanam agent create-from-role`; tenant isolation contract tests cover the new column; import-linter / AST enforcement; restored S25 docker-based clone-and-edit e2e against the role-aware shape (test body rewritten per the v3 methodology config + role authoring path; pre-write reconciliation: the skip decorator alone wasn't enough because the prior test body wrote a pre-v3 methodology config). Brief preserved at `briefs/p8/s26a-2.md`.

**S26b next.** McKinsey 7-Step methodology and its seven role aggregates land in the platform-managed methodology surface per `briefs/p8/mckinsey-7-step.md`. Authored against the role-first model from S26a; control-plane only, no per-tenant work expected.
