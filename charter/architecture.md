...
### Meta-classifier four-way routing

Per D140 (the meta-classifier dispatch). The `LlmMetaClassifierAdapter` routes an inbound message to one of four ConversationFlow surfaces — manual_entry, audit_conversation, mirror_conversation, and (S55b-2) **calendar_conversation** — plus the `dispatch_clarification` sentinel for the low-confidence case. The routing prompt + schema live at `shared_kernel/meta_classification.py` (the single-source-of-truth the production adapter and the D137 evaluation runner share); the `cell_identifier` enum and the `CellIdentifier` dispatch map at `contexts/messaging/application/dispatch_inbound.py` carry the fourth route. Calendar queries reach the S55b-1 cell through this path. D140 commits the extension to five-way at S56 (email_conversation). The read-only boundary holds at the routing layer: a calendar-*mutation* phrasing ("add a meeting to my calendar") has no write path (`calendar.readonly`, D148) — it routes to `calendar_conversation`, whose intent classifier returns `unclear_calendar` and the cell renders its clarification; calendar-write handling is deferred until a `calendar.write` scope is added (Phase 2-B or later).
