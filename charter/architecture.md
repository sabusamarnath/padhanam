...
Per D148/D149 (the calendar substrate) and the ConversationFlow primitive (D115). `contexts/calendar_conversation/` is the third ConversationFlow implementer (after audit-conversation 5.1 and mirror-conversation 4.1), mirroring their hexagonal shape: a domain intent surface (`parse_calendar_intent` over a discriminated calendar-intent union), an application cell consuming the structured-output, meeting-reader, confidence, threshold, and pending-clarification ports, a query builder filtering the Meeting store by intent, and a `CalendarConversationResponse` satisfying the `CitedResponse` Protocol (D138) by citing Meetings via `ArtefactCitation` with the `meeting` discriminator (D148). Resolution-ambiguity (a title reference matching multiple meetings) routes through D134's PendingClarification per D139, identically to the audit-conversation case-reference resolution. The cell cites the live Meeting search-cache row; the immutable citation-time audit-snapshot evidence record (the two-store split, D148) is wired at S55b-2 — see the citation-time audit-snapshot evidence sub-section below.

### Calendar-conversation refresh-before-answer

Per D150. The calendar-conversation cell refreshes the calendar at turn-open — invoking the D149 scoped full pull through `sync_calendar` via an injected refresh port — before querying the Meeting store, within the REAL_TIME_REQUIRED latency-tier budget (D122). Always-refresh (Option A) over staleness-windowed refresh because freshness is a must-have for an attentional tool and the measured refresh floor (~250 ms steady, 524 ms cold; S55b-1 close smoke against the rebuilt baked image, superseding the synced-source S55a-fix figures per D150) is imperceptible against the multi-second LLM cascade at single-personal-calendar volume. On refresh timeout (budget ~2000 ms, clearing the cold floor ~4×) or Nango/Google failure, the cell serves the cached Meeting store with a staleness note appended to the response rather than blocking or failing the turn — mirroring D146's always-produce-output composition discipline. Staleness-windowed and question-type-dependent refresh defer per the two-threshold rule (they optimise a redundant-pull cost that does not exist at 3 events and sub-400 ms); activation at Phase 2-B multi-user scale or when always-refresh latency breaches the tier budget. The refresh port is wired at the apps composition root to the real `NangoProxyCalendarAdapter` + `LiteLLMChunkEmbedder` + `Neo4jGraphRepository` driving `sync_calendar` (the consumer deferred from S55a).

### Meta-classifier five-way routing

Per D140 (the meta-classifier dispatch). The `LlmMetaClassifierAdapter` routes an inbound message to one of five ConversationFlow surfaces — manual_entry, audit_conversation, mirror_conversation, calendar_conversation (S55b-2), and email_conversation (S56b) — plus the `dispatch_clarification` sentinel for the low-confidence case. The routing prompt + schema live at `shared_kernel/meta_classification.py` (the single-source-of-truth the production adapter and the D137 evaluation runner share); the `cell_identifier` enum and the `CellIdentifier` dispatch map at `contexts/messaging/application/dispatch_inbound.py` carry the routes. The five-way band is tighter than the four-way — email "what came in" vs calendar "what's on" vs mirror "what's the status" — so the five-way routing gold set budgets extra deliberately-ambiguous entries and a higher `dispatch_clarification` rate is correct behaviour, not a regression. The read-only boundary holds at the routing layer for both substrate surfaces: a calendar-mutation phrasing routes to `calendar_conversation` and returns `unclear_calendar` (no `calendar.write` scope), and an email-mutation/send phrasing routes to `email_conversation` and returns `unclear_email` (`gmail.readonly`, no write/send scope); write/send handling is deferred until a write scope is added (Phase 2-B or later).

### Citation-time audit-snapshot evidence

Per D148 option b (the two-store evidence split), wired at S55b-2. A `Meeting` is the platform's **first mutable cited source**: the live row is a mutable search cache overwritten on each refresh (D149), with no per-version history (D148 rejected making Meeting Revisable). Every prior cited artefact — Case, DataPoint, audit events — is append-only or revision-with-lineage, so its own audit trail *is* the evidence and no citation-time snapshot is needed. Because the Meeting row mutates, citing it requires freezing the evidence at citation time: when `calendar_conversation` composes a cited response, the cell emits a citation-emission audit event (`resource_type: meeting_citation`) whose `after_state` carries an immutable snapshot of each cited Meeting's payload (event id, title, start/end, location, attendees, organizer, status, content hash, source-updated time). The live row remains the mutable search cache; the audit snapshot is the immutable evidence record, frozen by the append-only hash-chained audit substrate (D102/D110) even as the next refresh overwrites the live row. **The mechanism is calendar-local, not a shared citation-evidence hook**: a shared abstraction is justified only by a second mutable cited source, and the next artefact (Email, S56) is append-only/immutable once received — not a second mutable case, so a shared hook would be the anticipatory-control the two-threshold rule forbids; it is deferred to the second mutable-source instance. References D148 (option b; the two-store split), D102/D110 (append-only hash-chained audit substrate carrying the snapshot), D131/D138 (CitedResponse; the snapshot complements the `cited_artefacts` pointer with the frozen payload).

### Email substrate

Per D151, mirroring the calendar substrate (D148/D149) with email-specific divergences. **Transport:** the Gmail API through the self-hosted Nango google-mail provider (proxy base `https://gmail.googleapis.com`, `gmail.readonly` scope, the same Bearer + provider-config-key + connection-id proxy adapter as calendar); IMAP/provider-agnostic transport defers to Phase 2-B per the two-threshold rule. **Pull shape:** a two-call N+1 — `users.messages.list` returns `{id, threadId}` stubs over a bounded window (`q=newer_than:30d`, paginated), then batched `users.messages.get(format=full)` per id (the per-message round-trip is the cost centre calendar's single `events.list` did not have, so the gets are batched/concurrent). **Store:** Email keyed on `message_id`; subject/body/from/to/cc/snippet D21 envelope-encrypted; message_id/thread_id/received_at/labels/history_id/content_hash plaintext metadata. **Chunk-store divergence (the largest from calendar):** email bodies are long, so they are chunked and embedded into an email-local `email_chunks` store (per-chunk `vector(768)`, HNSW), inheriting ingestion's `ChunkEmbedderPort.embed(chunks)` as an injected port but using an email-local chunker (ingestion's chunking is parser/Source-coupled, not a standalone tool); email owns its store, not ingestion's. **Sync:** full-pull-only at Phase 2-A; `history_id` (from `getProfile`) is the stored dormant incremental anchor, no incremental built (the D149 lesson). **Deletion:** set-diff over the window-scoped stored set (Gmail's bounded query excludes Trash, so a vanished in-window message tombstones — no showDeleted-style flag). **Cite-directly:** email content is immutable once received, so email cites directly with no citation-time snapshot (the two-store evidence split stays calendar-only). **Refresh (D152):** Phase 2-A ships **Option A (full-pull-in-turn)**, the calendar-proven default (D150), behind the email-conversation cell's refresh port; **background-sync-then-serve and incremental are deferred as measurement-gated optimizations** (decided against a representative-inbox floor + volume at dogfooding, not the reconnaissance estimate), each a port-implementation swap plus a trigger (reusing the S53/S54 BroadcastDispatch infra), not a cell refactor.

### Email-conversation surface

Per D151/D152 and the ConversationFlow primitive (D115), the fifth ConversationFlow implementer (S56b), mirroring `calendar_conversation`'s hexagonal shape: a domain email-intent surface (`parse_email_intent` over a discriminated union — find_by_date_range, find_from_sender, find_by_subject, find_recent, unclear_email), an application cell consuming the structured-output, email-reader, confidence, threshold, pending-clarification, and **refresh** ports, a query builder filtering the email store by intent, and an `EmailConversationResponse` satisfying `CitedResponse` (D138) by citing Emails via `ArtefactCitation` with the `email` discriminator **directly — no citation-time snapshot** (D151: email content is immutable, unlike the calendar Meeting). Resolution-ambiguity (a subject reference matching multiple emails) routes through `PendingClarification` per D139. **Refresh-before-answer (D152 Option A):** the cell refreshes at turn-open through the injected `EmailRefreshPort` (wired to `sync_email` full-pull-in-turn) within the D150 tier budget, falling back to the cached store with a staleness note on timeout/failure (D146 always-produce-output). The cell holds no pipeline detail — it consumes the refresh port and never reaches through to `sync_email`, so the background-sync optimization is an apps-composition wiring swap, not a cell change.
...
BroadcastResponse satisfies CitedResponse Protocol from D138. Citation fields populate per BroadcastFlow implementer per the implementer's semantic (daily-briefing cites `cited_intake_records` covering the day's IntakeRecords; threshold-briefing cites the affected Meeting as an artefact and the `BROADCAST_INITIATED` audit event of the `THRESHOLD_CROSSED` fire — see the threshold-briefing sub-section for why the evaluation reads the state store, not the audit chain, per D153).
...
Two trigger sources at P15. (1) Scheduled triggers fire via the HTTP trigger endpoint per D145; external scheduler (cron, systemd timer, Kubernetes CronJob per deployment topology) hits the endpoint on schedule — at S57 the scheduler fires the `SCHEDULED_EVALUATION` trigger (refresh-then-evaluate) as well as `DAILY_SCHEDULED`. (2) The ThresholdEvaluator is itself a BroadcastFlow implementer (registered under `SCHEDULED_EVALUATION`) that, on a rule match, emits `THRESHOLD_CROSSED` through the FireTrigger idempotency flow; that trigger routes to the threshold-briefing implementer. See the threshold-briefing sub-section (D153) for the state-store evaluation model.

BroadcastDispatch emits a `BROADCAST_INITIATED` audit event before invoking the registered BroadcastFlow implementer. The audit chain entry carries `trigger_id`, `trigger_type`, `tenant_id`, `user_id`, `triggered_at`, plus (S57) the trigger `metadata` in its after_state when non-empty — so a `THRESHOLD_CROSSED` fire records the crossing identity (`rule_id`, `google_event_id`, `cancelled_at`). The implementer's response cites the `BROADCAST_INITIATED` event for end-to-end chain traversability.

### Threshold-briefing + ThresholdEvaluator (D153)

The proactive arc on the BroadcastFlow machinery, in its own bounded context `contexts/threshold_briefing/` (holding both implementers and the configured-rules schema; the two-threshold rule keeps them one context until a second proactive surface forces a split). Two BroadcastFlow implementers in a two-stage chain:

1. **ThresholdEvaluator** (registered under the new `SCHEDULED_EVALUATION` trigger type, added additively to `BroadcastTriggerType` per D142). Its `fire` runs **refresh-then-evaluate**: it syncs the active-rule substrates through a consumer **refresh port** (calendar at Phase 2-A; the threshold context never imports `sync_calendar` — the apps adapter wires it, the D146 pattern), then **evaluates over the calendar state store** through a consumer **state-reader port** that returns threshold-local `MeetingState` projections (not calendar domain types). On each match it emits `THRESHOLD_CROSSED` through a consumer **emitter port** wired to the D147 FireTrigger flow (fired_triggers idempotency); it returns a `ThresholdEvaluationResponse` citing the matched meetings.
2. **threshold-briefing** (registered under `THRESHOLD_CROSSED`). Its `fire` reads the crossing from the trigger metadata, composes the briefing prose via a consumer **composer port** (LiteLLM adapter), renders to WhatsApp (D135), and dispatches via a consumer **notifier port** (`send_message` with `message_intent=BROADCAST_THRESHOLD_BRIEFING`).

**Why evaluate over state, not the audit chain (the S57 reconciliation correction).** The P15 framing asserted the evaluator polls the audit chain for state-change matches. It cannot: `sync_calendar` writes only to the meetings store (upsert/tombstone) and emits no audit events, so calendar cancellations/moves are not in the chain (the only calendar audit events are the conversation cell's `meeting_citation` snapshots). And **conflict/double-booking was never a state-change event** — it is a standing condition over the current meeting set, intrinsically a query. A single model (evaluate current state; both rules are state queries) beats a hybrid that emits cancel/move events but still special-cases conflict, and avoids pulling an `AuditPort` into the calendar substrate at P15 close. The audit chain records the **outcome** (the `BROADCAST_INITIATED` + `THRESHOLD_CROSSED` fire, with the crossing identity in after_state), preserving procurement traceability of *why* the briefing fired; only the evaluation *input* moved from chain to store. Full change-data-capture of substrate deltas into the chain is a deferred procurement-hardening question (`charter/deferred-decisions.md`).

**Phase 2-A rule set (restraint against the reverse Kano category).** Two calendar must-haves: **meeting cancelled** (a tombstoned meeting whose `cancelled_at` is in the scan window) and **conflict/double-booking** (two confirmed meetings overlapping). For a platform-*initiated* surface, signal-to-noise is the product — a low-signal threshold trains the user to dismiss briefings and collapses the surface's value — so the set is deliberately narrow; the new-meeting threshold (excluded until it distinguishes externally- from self-initiated, else reverse) and volume-count thresholds (noise) are excluded. The configured-rules schema supports the deferred shapes (email-sender, topic, externally-initiated-meeting, cross-source synthesis, **meeting-moved** — which needs prior-vs-current start retention, a calendar-substrate touch deferred to dogfooding) but Phase 2-A populates only the two must-haves; the rest are config-tuned at dogfooding. The `THRESHOLD_CROSSED` idempotency key is the crossing's derived-state identity (cancellation: `rule_id` + `google_event_id`; conflict: `rule_id` + the unordered event pair) — one brief per crossing, no double-fire across scans. The cancellation identity excludes `cancelled_at` and the cancellation window-match is lower-bound only (an S57 live-smoke finding: the calendar tombstone resets `cancelled_at` to the refresh time, and refresh-then-evaluate refreshes inside the scan, so `cancelled_at` churns past `window_end` — including it would re-brief the same cancellation every scan).
...
Idempotency key semantics differ per trigger_type. DAILY_SCHEDULED: date string in operator timezone (one row per tenant+user+day). MANUAL: null (the UNIQUE constraint accommodates multiple null values per Postgres semantics). THRESHOLD_CROSSED at S57: the derived-state crossing identity per D153 (cancellation `rule_id` + `google_event_id`; conflict `rule_id` + the unordered event pair), excluding `cancelled_at`. See the threshold-briefing subsection above (the idempotency key and the reconciliation that moved it off `matched_audit_event_id`). The generic idempotency_key column accommodates heterogeneous semantics without per-trigger-type schema variation.
