...
**The rule is enforced by tooling, not just stated.** Two checks make known leakage impossible: the mapping path is in `.gitignore`, and `scripts/check_scrub.py` (run `make scrub-check`; wired as a pre-commit hook via `make install-hooks` → `core.hooksPath=.githooks`) fails if any real name from the mapping's real-name column appears in a tracked file — tests included, which is the surface that leaked (a fixture seeded from a live email). Names scrubbed only in context (an ordinary word scrubbed only in its entity occurrences, e.g. a product name that is also a common word) sit under a `context`-marked mapping section the guard skips, so their legitimate word uses do not trip it. Residual, stated honestly: the guard catches only names already in the mapping; a brand-new real name is caught by writer judgment at write time, then added to the mapping so the guard covers it thereafter. The guard makes known leakage impossible and new leakage a one-lookup discipline; it does not make it zero-judgment.

Origin of the tooling: S103ai. The scrub-before-publish gate had failed repeatedly by silent omission (real company/contact/goal names in session logs, and — worse — real data seeded into test fixtures), surfaced when the repo was found public with the leak. The remediation rewrote history to placeholders and, so it could not silently recur, turned the "tree-wide check" from an aspiration into an actual pre-commit guard. The deeper lesson the episode taught: real runtime data belongs only in the datastore; documentation references it via placeholders at write time, and test fixtures are synthetic — never seeded from live data.
...
The discipline: any acceptance criterion that asserts a path *works* must include a pass that runs it against the real surface it will run against in production — the actual Postgres constraint, the actual browser render, the actual vendor integration. Where a live pass is impractical in the moment, an enforcement test that ties the code to the real surface's shape (e.g. asserting a table's CHECK constraint lists every enum value) is the minimum standing guard, so the drift fails in unit tests rather than in production.

This is the assess-not-replace platform's verification law, and it binds every session that claims a path works: the browser-interactive-verification rule for UI surfaces (CLAUDE.md) is the same principle for the render surface; this generalises it to every real surface.

Origin: the second instance of one failure class. First: the S4 Langfuse-trace UI passed CLI smoke while the user-visible surface was broken (the browser-verification lesson). Second: the S91a/S91b matcher loop was "proven on a fixture" while `optimization run` was live-broken — the in-memory fake repositories never exercised the live `recommendations_category_check` constraint, so the matcher recommendation failed to persist and the loop could not run end-to-end until activation surfaced it. Two instances of one class earns the principle (the build-at-second-instance discipline). The first enforcement is the S91b-fix regression test tying the category CHECK to the `RecommendationCategory` enum; the corrective on the prompt side is that "prove the loop" must require a live persistence pass, not only fakes.

### The graph's meaning is authored, not only derived

A goal's causal structure — its levers, intermediaries, externals, and expected outcomes — is defined per goal, LLM-drafted and user-proofed, not inferred generically from signal. A generic graph, every goal carrying the same untyped relationships, loses meaning. The model co-evolves through one human-proof gate: the user authors and corrects, the system proposes new structure from emergent signal, and both directions pass through the user's proof. Human correction is the ground-truth signal the optimization layer learns from. Without it there is no learning. This is assess-not-replace carried to its conclusion: the system proposes, the human disposes, neither replaces the other.

Origin: the first live use of the S101 read-only Map on the real 8-goal corpus (D199) surfaced that an auto-derived, generic, uneditable CDD loses value as you look at it — the relationships carry no per-goal meaning and the user cannot correct them, so the model cannot learn, which guts the optimization differentiator. D200 records the pivot; this records the law it instances.

### Idempotent writes are tested against real persisted state, not a fresh mock

Any seed or merge that can re-run (an upload re-seed, an idempotent write) gets a persistence round-trip test: write, edit, re-write, assert the edit survived. A single-call mock returns a clean slate each call and cannot see a second write clobber the first. This closes the defect class that slipped the unit suite twice (the S103v dead-audit-port wiring and the S103af re-upload-reverts-edit bug).

Origin: S103af. The unit suite was green and the CV-upload round-trip's happy path passed, but only re-uploading the same CV *after an edit* — a second live call against real persisted state — exposed that the seed merge unconditionally `SET s.text`, reverting the operator's authored text. The mock proved the plumbing; only real prior state exposed that "idempotent" had a hole. This is a specialisation of "A proof on fakes is not a proof on the real surface": for a re-runnable write, the real surface that must be exercised is *the state a prior run left behind*.

### Data minimization on personal source documents

Persist the derived, proofed artefact, not the raw personal source, when the derived form is sufficient. The CV is parsed and the proofed skills profile stored, but the raw CV text is not persisted (S103af), because a proofed profile captures what downstream needs and keeping a full personal document in the graph as raw text is a standing privacy exposure for no proportionate benefit. This strengthens the enterprise data-handling posture: the platform holds derived structure, not users' raw personal documents.

Origin: S103af. Leg 1 (D236) stored the raw JD because leg 3 reads it as the role's side of the match; leg 2's CV, by contrast, is fully captured by the extracted-and-proofed `:SkillItem` profile, so the parsed text is dropped after seeding. `schema.md` offered a `:CvDocument` node as an option; not taking it is the lighter, more privacy-aligned call, and re-extraction is a re-upload away.

### Requirement and field sets are config-driven, never hardcoded dataclass attributes

New requirement types, qualification fields, or extracted attributes are rows in a config table read by a generic extractor, not new fixed dataclass fields or literal branches. This keeps the demand-side extraction a generic "extract N typed requirements from a spec" engine wearing domain labels at the surface, so a domain change is a config swap, not a code change. (Job hunt is the proof; the generic supply-demand engine is the bet.)

Origin: S103ad's `ExtractedQualification` — a fixed three-field dataclass (why_open / success_measures / selection_criteria) — proved the anti-pattern at S103ah: a real JD collapsed seven discrete essential criteria into a single `selection_criteria` blob, because a fixed field structurally cannot hold N discrete items. D240 replaces it with a config-driven requirement field set producing N discrete typed requirements. The law generalises: extend the config table, never the dataclass. The private assistant's whole positioning depends on the engine staying generic under domain labels, so the eventual private fork is a config relabel, not a rebuild (D184/D186, config over speculative structure).
