...
**The Map's first slice (D199).** How am I doing renders as a graph as well as a list, one source behind a toggle. Each goal is an outcome node carrying its verdict (`goal_status`, recomputed nowhere) and its measurable outcome, with the goal (the aim) shown distinct from the outcome (the measurable result) per D198. Its levers and its serving work feed it as sibling feeders, two hops deep, because the graph links work to the outcome through `SERVES`, not to a lever. It folds by default (D189, D190), keeps unlinked work in the coverage view (D193), and is read-only, writing no graph state. The intermediaries between feeders and outcome are shown as a missing link with one platform-limit note, never a per-goal verdict while the layer is absent (D198, D162, D171).
