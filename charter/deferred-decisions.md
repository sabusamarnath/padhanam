...
**Brief-preservation check at build-session close.** A check asserting that every closed build session has a preserved `briefs/<package>/<session>.md`, flagging the gap at session close rather than relying on the operator's framing habit to have produced one. Keys on the session-close signal (a new `log/sessions.md` entry, or a `feat(pN/sN)`-scoped build commit) and asserts the matching brief file exists. Surfaced from the Failure modes entry in `charter/methodology.md` (2026-06-15, brief-preservation lapse across S96–S98 — three build sessions shipped with no preserved brief, uncaught until the S97b framing session). Until the mechanical check lands, the operator's discipline is the framing habit plus a strategic-session spot-check of `briefs/` against recent sessions. Activation: routine pull from the upgrades backlog at a phase audit; the recurrence trigger the other candidates wait for has already fired (three consecutive silent lapses), so this one's activation case is met rather than pending.
...
**Measured result (S71 — the residual ran, the answer is do not build).** The S71 coverage report ran on the operator's real corpus after tiers one and two: **7/7 goals covered, 7 candidate edges, 982 orphan units across only 40 distinct titles.** The referential class — goal-work titled by a person/place/tool that lexical+alias cannot reach, *this tier's only target* — is **≤3 of those ~40 distinct items** (clearly one: a "Talent Screening … Lead Product Manager … Acme" interview belonging to Get-a-job; two networking events arguable). That is **below the build bar** (a handful, not a flood). The orphan volume is **96% recurrence inflation** (the top nine recurring daily events = 950 of 982 — a recurrence-fold problem, D173) plus **unseeded life areas** (health/medication, family/Megan, Esperanto — correctly uncovered, a goal-seeding choice). Neither is addressable by embeddings. **Decision: tier three is not built; the ≤3 referential cases close with an operator-declared link or one alias.**

**Activation trigger (raised bar).** Re-open only when a *re-measured* residual shows the referential class is a flood — and the honest re-measure waits until **recurrence-fold (D173) and operator goal-seeding have cleared the 96% noise**, because the current orphan read is dominated by inflation and missing goals, not by referential obliqueness. Until that re-measure crosses "more than a handful", the embedding tier stays deferred. The report (`make coverage-report`) is the standing instrument; this is the observability-needs-a-decision principle with the decision now recorded against the number. References D169 (the lexical matcher, the moat reads), D172 (tier one), D173 (recurrence-fold — must clear before a clean re-measure), D174 (the tier architecture + the residual trigger), D62 (the ingestion embedder, reusable when the bar is met).
