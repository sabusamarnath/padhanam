# Phase 2-A P13 framing brief commit

## Identification

Mode: strategic (charter commit; no code changes; lint and tests not required).
Block: pre-P13-framing brief landing. Brief authored at the Step 7 close thread per the briefs/ discipline pattern extended by analogy from arc-step framing (Steps 3-7 brief authored pre-conversation in prior step close thread) to package framing (P13 framing brief at Step 7 close thread). Substantive P13 framing conversation opens fresh after this commit lands.
Branch: operator-selected at session open.

## Goal at session close

- `briefs/p13/framing.md` created with the P13 framing brief verbatim per Commit 1.
- `briefs/p13/framing-brief-commit.md` preserves this commit prompt verbatim per Commit 2.
- Session log entry appended to `log/sessions.md` recording the brief landing per Commit 2.

## Context to read first

1. `briefs/phase-2/design-7step-step-7.md`. Step 7 brief as structural precedent for the package-framing brief shape (the brief absorbs the Step 7 structure adapted for package-framing altitude).
2. `briefs/phase-2/design-7step-step-7-brief-commit.md`. Step 7 brief commit prompt as session log shape precedent (two commits; no D-entries; documentation-only; reflection prompts answered).
3. `charter/current-package.md` tail. Confirm Step 7 close marker terminal; P13 framing brief commit appends a new close marker per the file's convention.
4. `log/sessions.md` tail. Latest Step 7 brief commit entry as shape precedent for this session log entry.
5. `charter/decisions.md` tail. Confirm no D-entries land at this commit (defer to P13 framing substantive conversation and its subsequent commit).

## Charter changes required this session

This commit lands the P13 framing brief plus its preservation only. No charter file modifications outside the brief creation plus commit prompt preservation plus session log entry plus the current-package.md close marker append.

## The substantive work

Two commits closing the session.

### Commit 1: P13 framing brief

Conventional commit message: `docs(briefs): p13 framing brief, pre-conversation landing`

Create `briefs/p13/framing.md` with the following content verbatim:
...
### Commit 2: Commit prompt preservation, spec preservation, current-package close marker, session log entry

Conventional commit message: `docs(charter): preserve p13 framing brief commit prompt; preserve spec; current-package close marker; session log`

Four artefacts.

**(a) Create `briefs/p13/framing-brief-commit.md`** with this commit-session prompt verbatim.

**(b) Create `docs/notes/spec-private-assistant-platform.md`** with the operator-supplied product specification verbatim. Source file at `/mnt/user-data/uploads/PRODUCT_SPECIFICATION.md`. The spec lands as `docs/notes/` reference material per Decision 6 option (c) of the brief; cross-reference target for the P13 framing substantive conversation and for Phase 2-B and Phase 3+ framing. Add a header note at the top of the file:
...
[full spec content verbatim from /mnt/user-data/uploads/PRODUCT_SPECIFICATION.md]
`````

**(c) Append to `charter/current-package.md`** a new close marker paragraph after the prior Step 7 close marker. Suggested text (adjust dates at write time):

`````markdown

**Phase 2-A P13 framing brief landed** at [DATE]. Brief preserved at `briefs/p13/framing.md` carrying seven pre-conversation decisions (substrate-completion carryover; hygiene workitems; session granularity at four sessions; roadmap v6 entry; Phase 2 PRD shape; spec-as-architectural-input commitment; forward-compat substrate-depth classification with build-now plus defer-with-trigger plus flag-for-future-testing categories); three substantive work-streams (epic note authoring; first-session prompt drafting; roadmap plus PRD plus spec-extracted charter additions); six reflection prompts at session close. Operator-supplied Private Assistant Platform spec preserved at `docs/notes/spec-private-assistant-platform.md`. Phase 2-A P13 framing substantive conversation opens at Claude.ai in a fresh thread per the briefs/ discipline pattern extended by analogy from arc-step framing (six-instance evidenced) to package framing (first instance).
`````

**(d) Append to `log/sessions.md`** a new entry matching the Step 7 brief commit entry shape, scaled for two commits plus the additional artefacts. Suggested structure (adjust dates and SHAs at write time):

`````markdown
## [DATE] — Phase 2-A P13 framing brief commit
...
1. `briefs/p13/framing.md` exists with the P13 framing brief verbatim per Commit 1; content matches the embedded block byte-for-byte except for any DATE placeholders the agent fills in at write time.
2. `briefs/p13/framing-brief-commit.md` exists with this commit-session prompt verbatim per Commit 2.
3. `docs/notes/spec-private-assistant-platform.md` exists with the header-note framing per Commit 2(b) plus the operator-supplied product specification content verbatim from `/mnt/user-data/uploads/PRODUCT_SPECIFICATION.md`.
4. `charter/current-package.md` carries a new close marker paragraph appended after the prior Step 7 close marker; append-only operation preserves prior content unchanged; close marker names P13 framing brief landing per Commit 2(c).
5. `log/sessions.md` carries the new session entry matching the Step 7 brief commit entry shape, scaled for two commits plus the additional artefacts, with the six reflection prompts answered and two methodology lines per Commit 2(d); close marker named at the entry's final line.
6. Two commits land in the order specified with conventional-commit messages as specified.
7. No content in `charter/decisions.md`, `charter/principles.md`, `charter/architecture.md`, `charter/packages.md`, `charter/deferred-decisions.md`, `charter/phase-2-design-7step.md`, `charter/methodology.md`, `charter/roadmap.md`, `charter/prfaq.md`, `charter/schema.md`, `charter/phase-2-user-segment.md`, `charter/competitors.md`, or any other charter file modified at this commit. D-entries and substantive charter additions defer to the Claude Code commit session following P13 framing substantive conversation.
8. Session log entry uses real-SHA-for-prior-commits plus "this commit" for self-reference per the same-commit-SHA-self-reference methodology line carried from the pre-Step-7 hygiene commit.

## Out of scope

- P13 framing substantive conversation (opens at Claude.ai in a fresh thread after this commit lands).
- D-entries from P13 framing (defer to the Claude Code commit session following P13 framing substantive conversation).
- Charter file modifications beyond brief preservation, spec preservation, current-package.md close marker append, and session log entry (defer to the same subsequent commit session).
- P13 epic note creation at `charter/packages/p13-epic.md` (Work-stream 1 of P13 framing substantive conversation; lands at the subsequent commit session).
- P13 first-session prompt drafting (Work-stream 2 of P13 framing substantive conversation).
- Roadmap v6 entry at `charter/roadmap.md` (Work-stream 3 of P13 framing substantive conversation).
- Phase 2 PRD section at `charter/prfaq.md` (Work-stream 3 of P13 framing substantive conversation).
- Spec-extracted charter additions at `charter/principles.md`, `charter/decisions.md`, `charter/deferred-decisions.md`, `charter/architecture.md` (Work-stream 3 of P13 framing substantive conversation).
- Pre-write reconciliation of S34 D98 principal shape for Decision 7's principal-pending-verification entry (P13 framing substantive conversation runs the reconciliation).
- Browser-based authentication substrate-completion session (Decision 1 option (a); follows P13 framing substantive conversation and precedes P13 Wave 1 first build session).
- Frontend stack confirmation strategic-mode block (Decision 1 option (a); follows P13 framing substantive conversation and precedes P13 Wave 1 first build session).
- Phase 2-entry hygiene session bundling four hygiene workitems (Decision 2 option (a); follows P13 framing substantive conversation; sequencing against browser-auth substrate-completion session settles at the hygiene session's framing).
- Phase 3 strategic-mode framing.
- Revision of the eight-package Phase 2 structure committed at Step 6.
- Publication-shaping work for the design 7-Step arc case study (follows Phase 2-A close per Step 7 brief out-of-scope).

## Session log entry instruction

After the second commit, append the strategic-block entry to `log/sessions.md` with the two reflection prompts and two methodology lines per Commit 2(d). Include `roles:` tag (technical writer, PM, analyst; analyst included given the spec-absorption work plus the forward-compat substrate-depth classification carry substantive analytical content). Use real-SHA-for-prior-commit plus "this commit" for the second commit's self-reference per the discipline carried from the pre-Step-7 hygiene commit's methodology line on same-commit-SHA self-reference.
