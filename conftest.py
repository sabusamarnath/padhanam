...
The Quorum top-level package is named ``platform`` (per D16 and
...
naming. We do not import stdlib ``platform`` anywhere in Quorum code, so
...
if _existing is not None and not hasattr(_existing, "__path__"):
    ...
    import platform  # noqa: F401  # re-imports as the Quorum platform package
