...
## 2026-06-02 — [S56a] Gmail vendor findings (two-call N+1, two-axis incremental gotcha, set-diff-not-showDeleted) + two build-time corrections

Source: the S56 strategic reconnaissance (verified against current Gmail API docs + the live Nango google-mail provider) and the S56a pre-write reconciliation. Recorded so the email substrate's vendor specifics are durable and the S56b/incremental-graduation sessions inherit them.

- **Two-call N+1 pull.** `users.messages.list` returns `{id, threadId}` stubs over a bounded window (`q=newer_than:30d`, paginated); content requires a follow-up `users.messages.get(format=full)` per id. Calendar's single `events.list` returned full events; email's per-message round-trip is the cost centre, so the gets must be batched/concurrent — serial gets over thousands of messages is the failure mode. This is why the full-pull floor (and the deferred refresh-strategy decision) is measured at S56a.
- **The two-axis incremental gotcha (the email D149-shape).** Incremental is `history.list(startHistoryId)`, and it has two independent fragilities: (a) startHistoryId expires to **HTTP 404 → mandatory full resync**, and historyId retention is "typically at least one week but may be significantly less or unavailable" — more fragile than calendar's syncToken 410; (b) `history.list` is **mailbox-wide and ignores any window**, so a bounded full pull does not bootstrap a window-scoped cursor and deltas must be window-filtered client-side. Documented now in D151's deferred-incremental clause so the incremental-graduation session builds against it, not after it (the D149 lesson: surface the vendor gotcha in reconnaissance, not mid-build).
- **Deletion is set-diff, not showDeleted.** Gmail's default query excludes Trash/Spam (`includeSpamTrash=false`), so a trashed message simply drops out of the next bounded pull with no `status=cancelled`-style flag (calendar had `showDeleted=true`). Email deletion detection is therefore set-diff over the window-scoped stored set — the very fallback D149 named but calendar never needed.

Two pre-write reconciliation corrections to the S56a prompt's assumptions (non-blocking, both sharpen the design):
- **historyId anchor source.** The prompt said "confirm `history_id` is on the list response." It is not — the list returns stubs only; the mailbox historyId comes from `users.getProfile` (or a message GET). The substrate anchors on `getProfile`'s historyId at pull time. Stored as plaintext metadata; no incremental built.
- **"Ingestion's chunker" is not a standalone tool.** Chunking is embedded in ingestion's format-specific parsers (`plain_text_parser`/`markdown_parser`), each coupled to ingestion's `Source` domain object. So email inherits only the **embedder** (`ChunkEmbedderPort.embed(chunks)`, a clean injected port, the substrate-inheritance discipline) and builds a small **email-local body chunker** rather than constructing a synthetic `Source` to borrow a parser. This keeps the injected-tools boundary clean (the answer to S56a reflection prompt 2).
...
## 2026-06-03 — [P15-audit-residue] Reconciliation-correction residue + the latent fallback that re-introduced a fixed bug

Source: the P15 audit (`docs/audit/p15-audit-evidence.md`) and this corrective follow-through session.

Reconciliation-correction residue. D153's audit-chain→state-store correction (S57) reached the code SSOT (`rule_match.py`) and the rewritten epic/architecture prose, but the pre-correction `matched_audit_event_id` / `cancelled_at` shape survived in five secondary surfaces (schema.md, architecture.md `:487` contradicting its own `:455`, deferred-decisions.md `:787`, `broadcast_flow.py` docstring, `idempotency.py` docstring+fallback) plus a test fixture, surfacing only at a dedicated audit pass. **Candidate: a correction-sweep checklist** — grep every surface carrying the old shape before closing a correcting session — distinct from pre-write reconciliation, which guards the *new* code; this guards the *old* shape's removal. Its checkable tell: grep the old shape's identifier across `contexts apps shared_kernel tests charter` before closing any session whose deliverable is a correction. Recurrence test: whether a third reconciliation correction leaves similar residue (D149 was the first reconciliation, D153 the second; this candidate promotes at a third instance or on structural-novelty grounds at the Phase 2-A close audit).

Adjacent observation (the latent fallback). The `idempotency.py` fallback (`_threshold_crossed_key`) was a green-suite-invisible latent re-introduction of the exact `cancelled_at`-churn bug the S57 live smoke caught: the unit test `test_threshold_crossed_falls_back_to_composite_when_identity_absent` *asserted* the buggy `rule_id:event:cancelled_at` shape, so the green suite locked the bug in rather than catching it. This is the **dead-or-defensive-path-carries-a-latent-bug** shape — a green that does not exercise the live path — sibling to the stale-bytecode enforcement masking and the S57 tombstone churn, and a second face of the **live-substrate-verification-gate** candidate already held for the Phase 2-A close audit. Reconciliation finding (recorded for the audit trail): the fallback is *not* dead code — the HTTP trigger endpoint (`apps/api/routers/triggers.py`) accepts a caller-supplied THRESHOLD_CROSSED metadata dict via `FireTriggerRequest`, so `crossing_identity` can be absent on that path; the fix therefore *reconstructs* the stable identity (mirroring `RuleMatch.crossing_identity()`, never embedding `cancelled_at`) rather than deleting the branch, and a unit test now binds the resolved key to the `RuleMatch` SSOT.

  - triaged: 2026-06-03
  - resolution: code + test fixes applied this session (`contexts/messaging/domain/idempotency.py`, `tests/unit/contexts/messaging/domain/test_idempotency.py`); docstring/fixture residue swept from `shared_kernel/{broadcast_flow,conversation_flow}.py` and `tests/unit/shared_kernel/test_broadcast_flow.py`; D155 sync notes added. The **correction-sweep checklist** and the **live-substrate-verification-gate** candidates remain pending for the Phase 2-A close audit (strategic mode), per the candidate-promotion bar.

## 2026-06-04 — [S58] Open-into-cell bug: the live-substrate lesson reaches the browser boundary

Source: the S58 daily-driver first-slice live run against the running stack. The operator surface's open-into-cell rendering assumed a *flat* case-detail response (`detail.title`, `detail.status`); the real `CaseDetailDTO` nests under `{case: {title, status, ...}, data_points: [{current_value, ...}]}`. Unit and HTTP-route tests were green (they assert the routes the surface *calls*, not how the surface *renders* the response), and CLI smoke would not catch it either — only loading `/app` in a real browser and clicking **Open** surfaced it. Fixed live (`50ac356`, `fix(p16/s58)`), then re-verified live including the `data_points[].current_value` path (not just re-committed).

- **This is the fourth instance of the live-substrate-verification lesson** after (1) the S4 Langfuse trace-UI display correctness, (2) the S55a-fix calendar syncToken vendor-contract falsification, and (3) the S57 threshold cancellation `cancelled_at`-churn — **and it is a new face of it.** The first three were backend or vendor-API; this is a **UI-to-API contract** that neither unit tests, contract tests, nor CLI smoke can catch, because the failing surface is the *rendering layer in a browser*. The fix needing its own live re-run (a commit is not a verification) is the same lesson applied to the repair.
- **Strengthens the held live-substrate-verification-gate candidate** (already pending for the Phase 2-A close audit per the [P15-audit-residue] entry) with an explicit new clause: **the gate extends to the browser boundary, not only backend smokes** — a UI acceptance criterion is met by a real browser pass, never by green tests plus a commit. This is structural novelty (a new failure-surface face), not mere recurrence, so it earns its place in the candidate rather than just incrementing a count.
...
## 2026-06-08 — [S66] Live data distribution falsified a correlation matcher that all synthetic tests passed (sixth instance, a new face)

Source: the S66 live `make correlate-units` against the personal tenant's real calendar cache. The title-and-time matcher grouped facets by normalised title and put every facet of a group into one unit. On synthetic test data (distinct titles, or one facet of each type) this is correct and all 10 matcher unit tests passed. On real data it was wrong: a recurring calendar event ("3 mins Esperanto with Megan", "Weekly standup", medication reminders) has ~84 instances that share an identical title, so the matcher collapsed all 84 same-type instances into a single 84-facet "unit" — the opposite of the high-precision, one-unit-of-work-per-D166 intent (a unit binds at most one facet *per type*; the cross-tool link is the differentiator, not same-type recurrence collapse).

- **Sixth instance of the live-substrate-verification lesson** after (1) S4 Langfuse trace-UI, (2) S55a-fix calendar syncToken vendor-contract, (3) S57 cancellation churn, (4) S58 browser-boundary UI-to-API, (5) S62 mock-cannot-validate-Cypher-params. **This is a new face: real *data distribution* falsified a pure-domain heuristic that synthetic tests passed.** The prior five were substrate behaviour, vendor contracts, runtime state-churn, a rendering-layer contract, and a mock's blindness to a parameter contract — all about the *substrate or the double*. This one is about the *shape of real data*: the matcher's logic was internally correct and fully unit-tested, but the test fixtures did not reproduce the real distribution (many same-title recurrences), so the green suite could not catch it. The fix (a unit binds at most one facet per type; same-type duplicates do not collapse; cross-type pairs by nearest time) was added with two new tests reproducing the real distribution, then live-re-verified (979 units, max 1 facet/unit, 0 duplicate-type units).
- **Strengthens the held live-substrate-verification-gate candidate** with a new clause: **the gate extends to real-data-distribution, not only substrate behaviour and vendor/mocked contracts** — a heuristic over ingested data is verified by running it against a representative real corpus, never by synthetic fixtures alone, because the fixtures encode the author's assumed distribution. This is structural novelty (a new failure-surface face), so it earns a clause rather than a count-increment.
