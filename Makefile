...
# computed from zephyr/config/ (currently just LITELLM_OTEL_HEADERS).
...
# Pulls the default model resolved from zephyr/config/ (D15, D19). The
...
pull-model: derive-env
	@model=$$(uv run python -c "from zephyr.config import InferenceSettings; print(InferenceSettings().default_model)") && \
...
# endpoint through zephyr/config/ (D19), then sends a real chat
...
#
# Runs from inside the zephyr-api container so the migration script
...
# runtime deps.
migrate: derive-env
	$(COMPOSE) exec zephyr-api python -m ops.migrate

# Register the test set tenants (postgres-tenant-a, postgres-tenant-b)
# in the registry. Idempotent: skips already-registered ids.
# Runs inside zephyr-api so the Compose service hostnames resolve.
seed-tenants: derive-env
	$(COMPOSE) exec zephyr-api python -m ops.seed_tenants
