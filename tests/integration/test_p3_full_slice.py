...
The audit row verification therefore runs *inside* the zephyr-api
...
from zephyr.config import ObservabilitySettings
...
def _api_base() -> str:
    return os.environ.get("ZEPHYR_API_BASE", "https://localhost/api")
...
from zephyr.observability.security_events import file_security_event_logger
...
def _seeded_tenants_present() -> bool:
    ...
    result = subprocess.run(
        [
            "docker", "compose", "exec", "-T", "zephyr-api",
            "python", "-",
        ],
        cwd=os.environ.get("ZEPHYR_REPO_ROOT", os.getcwd()),
...
from zephyr.config import TenantPostgresSettings
...
def _audit_count_for_tenant(tenant_uuid: str) -> int:
    """Run a SELECT COUNT(*) inside the zephyr-api container against
...
def stack_ready() -> None:
    if not _stack_reachable():
        pytest.skip("zephyr-api not reachable at " + _api_base())
