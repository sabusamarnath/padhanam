"""Enforce D19's "no os.getenv outside zephyr/config/" rule.
...
legitimate use in ``zephyr/security/crypto.py``. Banning ``os`` outright
is too coarse, so D19 is enforced here by AST walking instead. Any new
``os.getenv(...)`` call outside ``zephyr/config/`` fails this test.
"""
...
ALLOWED_PATH = REPO_ROOT / "zephyr" / "config"


def _source_files() -> list[Path]:
    roots = [REPO_ROOT / d for d in ("zephyr", "contexts", "shared_kernel")]
...
def test_no_os_getenv_outside_platform_config() -> None:
    ...
    assert offenders == [], (
        "os.getenv found outside zephyr/config/ (D19 violation): " + repr(offenders)
    )
